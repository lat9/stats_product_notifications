 ...
 */
  ...
  function products_notifications_count($products_id) {
    global $db;
    $customers = $db->Execute("SELECT count(*) as count
                               FROM " . TABLE_PRODUCTS_NOTIFICATIONS . "
                               WHERE products_id = " . $products_id);
    return($customers->fields['count']);
  }

  function products_notifications_customers($products_id) {
    global $db;
    $customers_array = array();
    $customers = $db->Execute("SELECT pn.customers_id, pn.date_added, c.customers_firstname, c.customers_lastname, c.customers_email_address
                               FROM " . TABLE_PRODUCTS_NOTIFICATIONS . " pn
                                 LEFT JOIN " . TABLE_CUSTOMERS . " c ON pn.customers_id=c.customers_id
                               WHERE pn.products_id = " . $products_id);
    while(!$customers->EOF) {
    	$customers_array[] = $customers->fields;
    	$customers->MoveNext();
    }
...
<?php
  ...
  $rows = 0;
  $order_by = "pn.products_id ASC";
  $products_query_raw = "SELECT distinct pn.products_id, pd.products_name
                        FROM " . TABLE_PRODUCTS_NOTIFICATIONS . " pn
                          LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd ON (pn.products_id = pd.products_id)
...
?>
              <tr class="dataTableRow" onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);">
                ...
                <td class="dataTableContent" align="center"><?php echo products_notifications_count($products->fields['products_id']); ?>&nbsp;
                <?php $showHide_cnt++; ?>
...
<?php
$customers_array = products_notifications_customers($products->fields['products_id']);
for ($i=0,$n=sizeof($customers_array);$i<$n;$i++) {
?>
                  <tr>
                    <td class="dataTableContent" align="left" width="50%"><a href="<?php echo zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $customers_array[$i]['customers_id'] . '&action=edit'); ?>" target="_blank"><?php echo $customers_array[$i]['customers_firstname'] . ' ' . $customers_array[$i]['customers_lastname']; ?></a></td>
                    <td class="dataTableContent" align="left" width="25%"><a href="<?php echo zen_href_link(FILENAME_MAIL, 'cID=' . $customers_array[$i]['customers_id'] . '&customer=' . $customers_array[$i]['customers_email_address']); ?>" target="_blank"><?php echo $customers_array[$i]['customers_email_address']; ?></a></td>
                    <td class="dataTableContent" align="left width="25%""><?php echo $customers_array[$i]['date_added']; ?></td>
                  </tr>
<?php
}
?>
